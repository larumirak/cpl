...
function targetsToChat_dialog() {
  ...
  // Build Text
  let content = "<b>Текущие цели: </b> ";
  if( targetedTokens.length == 0) content += "нет";
  ...
  content += "<br><br>Выбрать только одну случайную цель из текущего выбора?<br><br>";
  ...
  let dialog =  new Dialog({
      ...
      buttons: {
        yes: {
          icon: '<i class="fas fa-check-circle"> да</i>',
          label: "",
          callback: () => { this.targetsToChat(targetedTokens, true) }
        },
        no: {
          icon: '<i class="fas fa-times-circle"> нет, отправить их все</i>',
          label: "",
...
function targetsToChat(targetedTokens, pickRandom = false) {
  ...
  let flavor = "";
  if(pickRandom) {
    flavor = "Выбирает случайную цель";
