...
async function enterCombatAndRollInitative()
{
	...
	*/
    ...
    ui.notifications.error("Невозможно бросить инициативу: нет активного столкновения");
		return;
	}
...
{
  if(!game.combat) {
    ui.notifications.warn("Нет активного столкновения, нельзя пропустить ход");
    return;
  }
    ...
    ui.notifications.warn("Это не твой ход");
  }
}
...
function setHP_dialog() {
  ...
  let content = "<b>Затронутые токены: </b> ";
  let firstFlag = true;
  ...
  content += `<br><br><div class="form-group">
                    <label><b>Значение</b> </label>
                    <input name="hpvalue" value="0" placeholder="Value"/>
                  </div><br><b>Примеры:</b> +5, -5, = 5. Если оператор не определен: будет использоваться оператор +.`;
  ...
  let dialog =  new Dialog({
        ...
        buttons: {
          hp: {
            icon: '<i class="fas fa-heart"> Установить ХП</i>',
            label: "",
            callback: html => {
          ...
          temphp: {
            icon: '<i class="fas fa-shield-alt"> Установить врем. ХП</i>',
            label: "",
            callback: html => {
              let valString = html.find('input[name="hpvalue"]').val();
